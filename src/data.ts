...
export function buildSeed(t0: number): Seed {
  const people: Person[] = [
    ...
    },
    {
      ...
      role: 'vendor',
      kind: 'commercial',
      note: 'Sub-crew painter — helps on bigger jobs, invoices after each one.',
      tags: ['sub-crew'],
      suggestedTags: ['insured'],
      nbas: [{ id: 'nba_luis_w9', label: 'Collect W-9 for year-end 1099' }],
    },
  ];
  ...
  const history: Signal[] = [
    ...
    sig('p_luis', 'sms', t0 - 5 * DAY - 6 * HOUR, 'Confirmed for the Alder St prep — I can take both days. Same rate as the Fairview job.'),
    sig('p_marcus', 'sms', t0 - 3 * DAY - 5 * HOUR, 'Invoice 218 paid this morning. Thanks for the quick turnaround on Alder St.'),
    sig('p_luis', 'sms', t0 - 3 * DAY - 2 * HOUR, 'Wrapped the Alder St prep — walls patched and sanded, ready for your crew Monday.'),
  ...
  const script: ScriptedEvent[] = [
    ...
    },
    {
      ...
      signal: sig(
        ...
        'email',
        0,
        'Invoice attached for the Alder St prep — $1,240 for the two days. Whenever works in the next two weeks.',
        true,
      ),
    },
  ];
