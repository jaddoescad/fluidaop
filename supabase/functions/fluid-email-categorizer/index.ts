...
function authorized(req: Request): boolean {
  const expected = Deno.env.get('FLUID_EMAIL_CATEGORIZER_SECRET')?.trim() ?? '';
  const supplied = req.headers.get('x-fluid-agent-secret')?.trim() ?? '';
  return expected.length >= 43 && supplied.length >= 43 && safeEqual(expected, supplied);
}
...
function cleanAttachment(value: unknown): JsonRecord | null {
  ...
  const extracted = typeof value.extractedText === 'string'
  ...
  try {
    ...
    if (req.method === 'GET' && action === 'status') {
      ...
      const [counts, recentResult] = await Promise.all([
        ...
        supabase
          .from('agent_runs')
          .select('id,status,model,prompt_version,error,started_at,finished_at,activity_id')
          .eq('agent_key', AGENT_KEY)
    ...
    if (action === 'claim') {
      ...
      const limit = integer(body.limit ?? 1, 1, 5);
      const leaseSeconds = integer(body.leaseSeconds ?? 1800, 60, 3600);
      ...
      for (let index = 0; index < limit; index += 1) {
        const { data, error } = await supabase.rpc('claim_email_categorizer_job', {
          p_worker: worker,
    ...
    if (action === 'complete') {
      ...
      const labelKey = text(body.labelKey, 100, true);
      const confidence = typeof body.confidence === 'number' ? body.confidence : Number.NaN;
      const reason = text(body.reason, 1000, true);
      ...
      if (
        jobId === null || !validUuid(leaseToken) || labelKey === null ||
      ...
      const { data, error } = await supabase.rpc('complete_email_categorizer_job', {
        p_job_id: jobId,
    ...
    if (action === 'fail') {
      ...
      const model = text(body.model, 200) ?? '';
      const promptVersion = text(body.promptVersion, 100, true);
      if (jobId === null || !validUuid(leaseToken) || errorText === null || promptVersion === null) {
        return response({ error: 'Invalid failure payload' }, 400);
      }
      const { data, error } = await supabase.rpc('fail_email_categorizer_job', {
        p_job_id: jobId,
        p_lease_token: leaseToken,
        p_error: errorText,
        p_model: model,
