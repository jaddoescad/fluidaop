-- Report the full number of relevant People independently from the bounded
-- page. The Board can then say how many People exist without eagerly loading
-- every row merely to calculate a header badge.
...
comment on function public.list_real_board_people(text, integer, boolean, timestamptz, uuid) is
  'Lists activity-relevant People with cursor pagination and the total relevant count.';
