-- Keep the Board focused on sales relationships plus the internal team. Other
-- operational-only roles remain available elsewhere but do not appear here.
    ...
    where person.workspace_key = p_workspace_key
      ...
      and exists (
        ...
        where board_role.person_id = person.id
          and board_role.role_key in ('lead', 'customer', 'employee')
...
comment on function public.list_real_board_people(text, integer, boolean, timestamptz, uuid) is
  'Lists activity-relevant leads, customers, and employees for the Board People column with cursor pagination and a total count.';
