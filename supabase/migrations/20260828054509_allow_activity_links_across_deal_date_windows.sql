-- One communication may truthfully belong to more than one deal when their
-- date windows overlap. The previous activity-only primary key forced an
-- arbitrary exclusive choice and left valid communications unassigned.
alter table public.deal_activity_links
  drop constraint deal_activity_links_pkey;

alter table public.deal_activity_links
  add primary key (workspace_key, activity_id, deal_id);

alter table public.deal_activity_links
  drop constraint deal_activity_links_attribution_method_check;

alter table public.deal_activity_links
  add constraint deal_activity_links_attribution_method_check
...
begin
  ...
  -- Explicit provider deal ids remain the strongest evidence, but still must
  -- fall inside the deal's real date window.
  ...
  -- Deterministic boundary attribution. Overlapping windows intentionally
  -- create multiple links so the same communication is visible on both deals.
  insert into public.deal_activity_links (
    workspace_key, activity_id, deal_id, stage_event_id, stage_name,
    attribution_method, stage_evidence, confidence, reason, metadata
  )
...
-- Rebuild all automatic links under the boundary model.
do $$
...
comment on table public.deal_activity_links is
  'Evidence-bearing many-to-many activity attribution. Overlapping deal date windows may intentionally share an activity.';
