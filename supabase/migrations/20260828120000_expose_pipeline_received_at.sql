-- The board filter bar needs the date a lead actually arrived, not the date the
-- sync snapshot ran. captured_at is rewritten on every sync, so every current
-- deal shares the same value and cannot be grouped by month. Expose the same
-- date the deal journey already reports as "Deal created":
-- estimated_created_at when DripJobs gave us a deal age, else first_seen_at.
...
drop function if exists public.list_archived_dripjobs_pipeline(text, integer, timestamptz, text);
...
comment on function public.list_archived_dripjobs_pipeline(text, integer, timestamptz, text, date) is
  'Keyset-paginated archived DripJobs deals, optionally filtered by lead-received month.';
