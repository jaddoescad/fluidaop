-- The strip draws each day as a 12px square, and sixteen of them on a 2px gap
-- is what a 272px board column holds. The previous 28-day window only fit by
-- squeezing the cells into 6px slivers, which stopped reading as a day each.
-- Squares are the point, so the window gives way instead: sixteen days, then
-- the oldest fall off the left and `daysBefore` counts what was dropped.
...
as $$
  ...
  bounds as (
    select
      ...
      -- not started yet; clamping keeps the strip at one honest square.
      least(
  ...
  strip_day as (
    ...
    -- answer; a run of empty squares would be an accusation.
  ...
  select jsonb_build_object(
    ...
    'daysBefore', (
      select coalesce(greatest(0, (bounds.today - bounds.entered_on) - 15), 0) from bounds
    )
  )
  from touch;
$$;
...
comment on function private.deal_current_stage_touches(text, text) is
  'Touch counts, last-touch time and a day-per-square activity strip anchored to the day the deal entered its current stage.';
